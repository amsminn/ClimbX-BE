...
/**
 * OAuth2 ID Token 검증을 위한 Configuration
 * Spring Security OAuth2 Resource Server 기반
 */
@Slf4j
@Configuration
...
public class ProviderIdTokenConfig {
    ...
    /**
     * Provider별 JwtDecoder 맵을 생성합니다.
     * 각 Provider의 구현체에서 제공하는 설정을 사용하여 JwtDecoder를 동적으로 생성합니다.
     */
    @Bean
    ...
    /**
     * UserInfoExtractor에서 제공하는 설정을 사용하여 JwtDecoder를 생성합니다.
     * NimbusJwtDecoder가 공개키 캐싱 및 자동 갱신 기능 제공
     */
    private JwtDecoder createJwtDecoder(UserInfoExtractor extractor) {
        ...
        OAuth2TokenValidator<Jwt> audienceValidator = createAudienceValidator(extractor.getClientId());
    ...
    /**
     * Audience 검증을 위한 커스텀 Validator 생성
     * 재사용 가능한 검증기입니다.
     */
